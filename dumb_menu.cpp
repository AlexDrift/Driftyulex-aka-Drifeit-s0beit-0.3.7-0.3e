...
#define ID_MENU_PLAYERS_WARPVEH 301
...
#define ID_MENU_SAMPMISC_FAST_KILL			300
...
#define ID_MENU_SAMPMISC_ENABLEGTACHEATS	302
#define ID_MENU_SAMPMISC_PICKUP_PICKUP		303
...
static void menu_cheats_fast_kill_populate ( struct menu *menu )
{
	menu_items_free( menu );
	...
	for ( i = 0; i < SAMP_PLAYER_MAX; i++ )
	{
		...
		snprintf( text, sizeof(text), "Fast killed by %s (ID: %d)", getPlayerName(i), i );

		menu_item_add( menu, NULL, text, ID_MENU_SAMPMISC_FAST_KILL, color, (void *)(UINT_PTR) i );
	}

	int localId = g_Players->sLocalPlayerID;
	D3DCOLOR	color = samp_color_get( localId );
	menu_item_add( menu, NULL, "Suicide!!!", ID_MENU_SAMPMISC_FAST_KILL, color, (void *)(UINT_PTR) localId );
}

static void menu_cheats_enable_populate ( struct menu *menu )
{
	menu_items_free( menu );

	menu_item_add( menu, NULL, "Enable GTA Cheats", ID_MENU_SAMPMISC_ENABLEGTACHEATS, MENU_COLOR_DEFAULT, (void *)(UINT_PTR) 0 );
	menu_item_add( menu, NULL, "Disable show chatbox on T (e.g. ROCKETMAN)", ID_MENU_SAMPMISC_ENABLEGTACHEATS, MENU_COLOR_DEFAULT, (void *)(UINT_PTR) 1 );
}

static void menu_pickup_pickup_populate ( struct menu *menu )
{
	menu_items_free( menu );

	char text[32];
	sprintf_s( text, sizeof(text), "Pickup id: %d", set.sendPickupId );

	menu_item_add( menu, NULL, text, ID_MENU_SAMPMISC_PICKUP_PICKUP, MENU_COLOR_DEFAULT, (void *)(UINT_PTR) 0 );
	menu_item_add( menu, NULL, "Send!", ID_MENU_SAMPMISC_PICKUP_PICKUP, MENU_COLOR_DEFAULT, (void *)(UINT_PTR) 1 );
}
...
static void menu_players_warpveh_populate ( struct menu *menu )
{
	menu_items_free( menu );
...
static void menu_event_activate ( struct menu *menu )
{
	...
	case ID_MENU_PLAYERS_WARPVEH:
		menu_players_warpveh_populate( menu );
		break;
	...
	case ID_MENU_SAMPMISC_FAST_KILL:
		menu_cheats_fast_kill_populate( menu );
		break;
	...
	case ID_MENU_SAMPMISC_ENABLEGTACHEATS:
		menu_cheats_enable_populate( menu );
		break;

	case ID_MENU_SAMPMISC_PICKUP_PICKUP:
		menu_pickup_pickup_populate( menu );
		break;
	...
	case MENU_OP_ENABLED:
		{
			...
			case ID_MENU_SAMPMISC_ENABLEGTACHEATS:
				{
					int idx = (int)(UINT_PTR) item->data;
					switch ( idx )
					{
					case 0:
						return 0;
	...
	case MENU_OP_SELECT:
		{
			...
			case ID_MENU_SAMPMISC_FAST_KILL:
				{
					SendWastedNotification((BYTE)cheat_state->_generic.spoof_weapon, (SHORT)(UINT_PTR) item->data);
				}
				break;
			...
			case ID_MENU_SAMPMISC_PICKUP_PICKUP:
				{
					int idx = (int)(UINT_PTR) item->data;
					 BitStream bsSend;
					bsSend.Write(set.sendPickupId);
					g_DrifeitClient->RPC(RPC_PickedUpPickup, &bsSend, HIGH_PRIORITY, RELIABLE_ORDERED, 0, 0);
				}
				break;
			}
		}
		...
		case ID_MENU_SAMPMISC_PICKUP_PICKUP:
			{
				int idx = (int)(UINT_PTR) item->data;
				if (idx == 0) {
					set.sendPickupId += (int)mod;
					if ( set.sendPickupId < 0 || set.sendPickupId > 4095 ) set.sendPickupId = 0;
					menu_item_name_set( item, "Pickup id: %d", set.sendPickupId );
				}
			}
			break;
		}
...
static int menu_callback_players_warpveh ( int op, struct menu_item *item )
{
	if ( op == MENU_OP_SELECT )
	{
		bool vehicles = false;
		...
		float				pos[3];
		bool once			= false;
		struct vehicle_info * actor_vehicle = vehicle_info_get( VEHICLE_SELF, VEHICLE_ALIVE );
		 ...
		 ||	 g_Players->pRemotePlayer[id]->pPlayerData->pSAMP_Actor == NULL )
		{
			...
			if ( pool_vehicle != NULL )
			{
				for ( int i = 0; i < pool_vehicle->size; i++ )
				{
					struct vehicle_info			*info = vehicle_info_get( i, VEHICLE_ALIVE );

					if ( info == NULL )
						continue;

					if ( actor_vehicle == info )
						continue;

					int	id = getSAMPVehicleIDFromGTAVehicle( info );
					...
					cheat_vehicle_teleport(info, g_stStreamedOutInfo.fPlayerPos[id], gta_interior_id_get());
					SendFakeUnoccupiedSyncData( (USHORT)id, g_stStreamedOutInfo.fPlayerPos[id] );
				}
			}

			if ( once == false ) addMessageToChatWindow( "There is no empty vehicles in your location." );
		...
		if ( pool_vehicle != NULL )
		{
			for ( int i = 0; i < pool_vehicle->size; i++ )
			{
				...
				cheat_vehicle_teleport(info, pos, actor->base.interior_id);
				SendFakeUnoccupiedSyncData( (SHORT)id, pos );
			}
		}
	...
	struct menu *menu_main, *menu_cheats, *menu_cheats_mods, *menu_cheats_inv, *menu_cheats_money, *
		menu_cheats_weather, *menu_cheats_time, *menu_weapons, *menu_vehicles, *menu_teleports, *menu_interiors, *
			menu_misc, *menu_debug, *menu_hudindicators, *menu_patches, *menu_players, *menu_servers, *
				menu_players_warp, *menu_players_warpveh, *menu_players_vehwarp, *menu_players_spec,
#ifdef __CHEAT_VEHRECORDING_H__
	*menu_routes, *menu_routes_load, *menu_routes_drop, 
#endif

	//*menu_cheats_handling,
	*menu_player_info, *menu_players_mute, *menu_sampmisc, *menu_spoof_weapon, *menu_fake_kill, *menu_fast_kill, *menu_vehicles_instant, 
	*menu_gamestate, *menu_specialaction, *menu_teleobject, *menu_telepickup, *menu_samppatches, *menu_enable_cheats, *menu_pickup_pickup;
	...
	if ( g_dwSAMP_Addr != NULL )
	{
		...
		menu_players_warpveh = menu_new( menu_players, ID_MENU_PLAYERS_WARPVEH, menu_callback_players_warpveh );
		...
		menu_fast_kill = menu_new( menu_sampmisc, ID_MENU_SAMPMISC_FAST_KILL, menu_callback_sampmisc );
		menu_enable_cheats = menu_new( menu_sampmisc, ID_MENU_SAMPMISC_ENABLEGTACHEATS, menu_callback_sampmisc );
		menu_pickup_pickup = menu_new( menu_sampmisc, ID_MENU_SAMPMISC_PICKUP_PICKUP, menu_callback_sampmisc );
