...
void cheat_handle_vehicle_crasher(struct vehicle_info *vinfo)
{
if(KEY_PRESSED(set.key_crasher))
{
cheat_state->vehicle.crasher++;
if(cheat_state->vehicle.crasher > 1)
if(cheat_state->vehicle.crasher > 0)
cheat_state->vehicle.crasher = 0;
cheat_state->vehicle.modelid = 1000;
cheat_state_text( "Crashed." );
}
if(!cheat_state->vehicle.crasher)
return;
switch(cheat_state->vehicle.crasher)
{
case 1:
if((GetTickCount() - 1) < cheat_state->vehicle.crasher)
return;
switch(cheat_state->vehicle.crasher)
{
case 0:
sendSCMEvent( 2, g_Players->pLocalPlayer->sCurrentVehicleID, cheat_state->vehicle.modelid, 0 );
cheat_state->vehicle.modelid += 1;
break;
case 1:
cheat_state->vehicle.modelid += 1;
break;
...
cheat_state->vehicle.crasher = GetTickCount();
}
