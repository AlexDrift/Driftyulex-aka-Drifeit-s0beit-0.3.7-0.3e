...
CD3DFont				*pD3DFont = new CD3DFont( "Impact", 10, FW_BOLD );

//pd3dFont_sampStuff = player info list, player score list, player ESP
CD3DFont				*pD3DFont_sampStuff = new CD3DFont( "Tahoma", 9, FW_BOLD | FCR_BORDER );

//pD3DFontFixed = cheat_state_msg, HUD
CD3DFont				*pD3DFontFixed = new CD3DFont( "Impact", 8, FW_BOLD );
...
CD3DFont				*pD3DFontChat = new CD3DFont( "Florencesans SC Exp", 8, FW_BOLD | FCR_BORDER );

//pD3DFontDebugWnd = debug window
CD3DFont				*pD3DFontDebugWnd = new CD3DFont("a_AvanteBse", 8, FW_BOLD | FCR_BORDER );
...
void RenderDVD ( void )
{
	...
	pD3DFont->PrintShadow(left, top, D3DCOLOR_XRGB(rand()%255, rand()%255, rand()%255), "vk.com/id311966245 (Скайп: alexdrift----------)");
	top1 = pPresentParam.BackBufferHeight - 60;
	pD3DFont->PrintShadow(left1, top1, D3DCOLOR_XRGB(rand()%255, rand()%255, rand()%255), "Дрифтюлекс м0д от AlexDrift");
...
void RenderDVDa ( void )
{
	...
	left = 800;
	top = pPresentParam.BackBufferHeight - 19;
	pD3DFont->PrintShadow(left, top, D3DCOLOR_XRGB(rand()%255, rand()%255, rand()%255), "vk.com/id311966245 (Скайп: alexdrift----------)");
}
...
void RenderMenu ( void )
{
	...
	render->D3DBox(left - 2, y, 2, MENU_HEIGHT, D3DCOLOR_ARGB(223, 63, 63, 63));*/
...
void RenderPedHPBar ( void )
{
	...
	if ( info->armor == NULL )
	{
		_snprintf_s( text, sizeof(text)-1, "No Armour" );
	...
	while ( iter.pos < iter.end )
	{
		...
		if ( !g_playerTagInfo[iGTAID].isStairStacked )
		{
			...
			while ( iterInner.pos < iterInner.end )
			{
				...
				// test to see who comes out on topAlex
...
void renderVehicleTags ( void )
{
	...
	while ( iter.pos < iter.end )
	{
		...
		_snprintf_s( buf, sizeof(buf)-1, "%s (Number %d)", vehicle->name, v);
		w = pD3DFontFixed->DrawLength( buf );

		// different color if car is being driven
		DWORD	color_veh;
		if ( iterVehicle->IsBeingDriven() )
			color_veh = D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0); // blueish 100, 150, 235
		else
			color_veh = D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0);
		...
		vh /= 10.0f;
		if ( vh > 50.0f )
			vcolor = D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0);
		else if ( vh > 25.0f )
			vcolor = D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0);
		else
			vcolor = D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0);
		...
		_snprintf_s( buf, sizeof(buf)-1, "HP: %d", (int)vh );
...
void RenderPickupTexts ( void )
{
	...
	if ( self != NULL )
	{
		for ( i = 0; i < SAMP_PICKUP_MAX; i++ )
		{
			...
			_snprintf_s( buf, sizeof(buf)-1, "Pickup Number: %d, ModelID: %d", i, g_SAMP->pPools.pPool_Pickup->pickup[i].iModelID );
			pD3DFontFixed->PrintShadow( screenpos[0], screenpos[1] - 5.0f, D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), buf );
		}
	}
}
...
void RenderObjectTexts ( void )
{
	...
	for ( i = 0; i < SAMP_OBJECTS_MAX; i++ )
	{
		...
		_snprintf_s( buf, sizeof(buf)-1, "Object Number: %d, ModelID: %d", i,
...
void renderPlayerInfoList ( void )
{
	...
	if ( (GetAsyncKeyState(VK_TAB) < 0 && set.d3dtext_score)
	 ||	 *(int*)(*(DWORD *)(g_dwSAMP_Addr + SAMP_SCOREBOARD_INFO)) == 1 ) return;
	...
	for ( i = current_player_id; i < SAMP_PLAYER_MAX; i++ )
	{
		...
		{
			_snprintf_s( buf, sizeof(buf)-1, "Nmae: %s (ID: %d), No strim", getPlayerName(i), i );
...
void renderScoreList ()
{
	...
	*(int*)(*(DWORD *)(g_dwSAMP_Addr + SAMP_SCOREBOARD_INFO)) = 0;
	...
	_snprintf_s( buffer, sizeof(buffer)-1, "%s (%s:%d) Игроков на сервере: %d", g_SAMP->szHostname, g_SAMP->szIP,
		...
		if (bCollision && pCollision)
		{
			...
			if (pCollisionEntity && pCollisionEntity->nType == ENTITY_TYPE_VEHICLE)
			{
				...
				if (pCVehicleTeleport)
				{
					...
					{
						sprintf(buf, "Teleport to %s", vehicleEntry->name);
					}
				}
				else
				{
...
void renderChat ( void )
{
	...
	if ( (KEY_DOWN(VK_TAB) && set.d3dtext_score)
	 ||	 (*(int*)(*(DWORD *)(g_dwSAMP_Addr + SAMP_SCOREBOARD_INFO)) == 1 && !set.d3dtext_score) ) return;
...
void renderSAMP ( void )
{
	...
	if ( !g_renderSAMP_initSAMPstructs )
	{
		...
		// initialize raknet
		g_DrifeitClient = new DrifeitClient(g_SAMP->pDrifeitClientInterface);
		g_SAMP->pDrifeitClientInterface = new HookedRakClientInterface();

		

		// Anticrasher
		mem::set((PVOID)(g_dwSAMP_Addr + 0x5A1D9), 0x90, 5);
	...
	if ( g_SAMP != NULL )
	{
		...
		if ( g_SAMP->pSettings->byteNoNametagsBehindWalls && set.disable_line_of_sight )
			if (!set.m0d) g_SAMP->pSettings->byteNoNametagsBehindWalls = 0;
		if ( !g_SAMP->pSettings->bytePlayerMarkersMode && set.markers_show )
			if (!set.m0d) g_SAMP->pSettings->bytePlayerMarkersMode = PLAYER_MARKERS_MODE_GLOBAL;
...
void texturesInitResources ( IDirect3DDevice9 *pDevice, D3DPRESENT_PARAMETERS *pPresentationParameters )
{
	...
	{
		char filename[512];
		snprintf(filename, sizeof(filename), "title.png", g_szWorkingDirectory);
	...
	if (!proxyIDirect3DDevice9_init)
	{
		...
		*(uint8_t *)0xBAB233 = gta_money_hud->alpha;
		...
		if (!set.use_old_menu)
		{
			...
			twBar_Testing = TwNewBar("Новые_операции", 1);
			twBar_Testing1 = TwNewBar("Разные_операции", 1);
			twBar_Testing2 = TwNewBar("Пешие_операции", 1);
			twBar_Testing3 = TwNewBar("Автомобильные_операции", 1);
			twBar_Testing4 = TwNewBar("Пакетные_операции", 1);
			twBar_Testing5 = TwNewBar("NetBot_Панель", 1);
			twBar_Testing6 = TwNewBar("Версус_Протект", 1);
			twBar_SPCheats = TwNewBar("Читы", 1);
			twBar_SPCarUpgrades = TwNewBar("Vehicle_Upgrades", 1);
			twBar_SPCarColorPJ = TwNewBar("Vehicle_ColorPJs", 1);
			twBar_SPWeapons = TwNewBar("Оружие", 1);
			twBar_SPVehicles = TwNewBar("Машины", 1);
			twBar_SPTeleports = TwNewBar("Телепорты", 1);
			twBar_SPMisc = TwNewBar("Разное", 1);
			twBar_SPPatches = TwNewBar("GTA_Патчи", 1);

			twBar_SAMPPlayers = TwNewBar("Игроки", 1);
			twBar_SAMPFavServers = TwNewBar("Ваш_список_серверов", 1);
			twBar_SAMPMisc = TwNewBar("СА:МП_Разное", 1);
			twBar_SAMPObjects = TwNewBar("СА:МП_Обьекты", 1);
			twBar_SAMPPickups = TwNewBar("СА:МП_Пикапы", 1);
			twBar_SAMPPatches = TwNewBar("СА:МП_Патчи", 1);
		}
...
#define HUD_TEXT_TGL( x, color, text ) \
	...
	uint32_t	color_disabled = D3DCOLOR_XRGB(255, 165, 0);
	...
	if (isBeginRenderWIN)
	{
		...
		if (set.d3dtext_hud)
		{
			if (cheat_panic() || cheat_state->state == CHEAT_STATE_NONE)
			{
				...
				if (!set.flickering_problem)
				{
					if (iIsSAMPSupported)
					{
						...
						_snprintf_s(buf, sizeof(buf) - 1, "%s для %s", NAME, g_szSAMPVer);
					...
					{ 
						...
						render->D3DBoxi((int)y - 1,
							(int)(pPresentParam.BackBufferHeight - 1) - (int)pD3DFont->DrawHeight() - 3,
							(int)(pPresentParam.BackBufferWidth + 14), 22, bar_color, NULL);
						HUD_TEXT(y, D3DCOLOR_ARGB(127, 255, 255, 255), NAME);
			...
			else
			{
				...
				if (set.hud_draw_bar)
				{
					...
					/*Верхнее меню*/
					render->D3DBoxi((int)x - 105, (int)(pPresentParam.BackBufferHeight - 1) - (int)pD3DFont->DrawHeight() - 24, //-1)      
						(int)(pPresentParam.BackBufferWidth + 14), 22, bar_color, NULL); // +14)      
				
				}

				if (set.hud_indicator_inv)
				{
					HUD_TEXT_TGL(x, cheat_state->_generic.hp_cheat ? color_enabled : color_disabled, "ГМ");
				}
				...
				{
					HUD_TEXT_TGL(x, cheat_state->_generic.vehicles_freeze ? color_enabled : color_disabled, "Мороз");
				}
				...
				if (set.hud_fps_draw)
				{
					...
					{
						_snprintf_s(buf, sizeof(buf) - 1, "FPS: %0.0f", m_FPS, *(int *)0xC1704C);
					}
					else
					{
			...
			if (cheat_state->state == CHEAT_STATE_VEHICLE)
			{
				...
				{
					HUD_TEXT_TGL(x, cheat_state->vehicle.brkdance ? color_enabled : color_disabled, "БрейкДанс");
				}

				if (set.hud_indicator_inveh_spider)
				{
					HUD_TEXT_TGL(x, cheat_state->vehicle.spiderWheels_on ? color_enabled : color_disabled, "Спайдер");
				}

				if (set.hud_indicator_inveh_fly)
				{
					HUD_TEXT_TGL(x, cheat_state->vehicle.fly ? color_enabled : color_disabled, "Полет");
				}
			...
			else if (cheat_state->state == CHEAT_STATE_ACTOR)
			{
				...
				{
					HUD_TEXT_TGL(x, cheat_state->actor.fly_on ? color_enabled : color_disabled, "Полет");
				}
			...
			if (cheat_state->context_time > 0 && time_get() - cheat_state->context_time < MSEC_TO_TIME(3000))
			{
				if (g_SAMP->iGameState == GAMESTATE_CONNECTED)
				{
					...
					pD3DFont->PrintShadow(1, pPresentParam.BackBufferHeight - 42, //40
		...
		if (gta_menu_active)
		{
			TwInit(TW_DIRECT3D9, origIDirect3DDevice9);
			TwWindowSize(pPresentParam.BackBufferWidth, pPresentParam.BackBufferHeight);
			//если ГТА открыта - иницизируем AntTweakBar 
		}
		else
		{
			if (!set.use_old_menu)
			{
				TwTerminate();
				//если ГТА свернута - отключаем AntTweakBar 
			}
		}
		// render menus
		if (set.use_old_menu)
