...
static void ini_init ( void )
{
	...
	if ( (ent = ini_register_entry("fps_limit", TYPE_INT)) != NULL )
		ini_register_data( ent, &set.fps_limit, "300" );
	...
	if ((ent = ini_register_entry("rapid_speed", TYPE_FLOAT)) != NULL)
		ini_register_data(ent, &set.rapid_speed, "5.0");

	if ((ent = ini_register_entry("rapid_speedd", TYPE_FLOAT)) != NULL)
		ini_register_data(ent, &set.rapid_speedd, "1463.99, -1720.52, 13.80");
...
ini_register_data( ent, set.admin + i, "" ); 
	...
	*/
	...
	/*Carspam*/  
  if ( (ent = ini_register_entry("key_carspam", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_carspam, "" );

  /*Crasher*/  
  if ( (ent = ini_register_entry("key_crasher", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_crasher, "" );

  /*fake*/  
  if ( (ent = ini_register_entry("key_fake", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_fake, "" );

  /*skin_changer*/  
  if ( (ent = ini_register_entry("key_skin_changer", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_skin_changer, "" );

  /*respavner*/  
  if ( (ent = ini_register_entry("key_respavner", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_respavner, "" );

  /*pic*/  
  if ( (ent = ini_register_entry("key_pic", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_pic, "" );

  /*rcon*/  
  if ( (ent = ini_register_entry("key_rcon", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_rcon, "" );

   /*ddos*/  
  if ( (ent = ini_register_entry("key_ddos", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_ddos, "" );

  /*pulsedisk*/  
  if ( (ent = ini_register_entry("key_pulsedisk", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_pulsedisk, "" );

  /*pulsedweapon*/  
  if ( (ent = ini_register_entry("key_pulsedweapon", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_pulsedweapon, "" );

  /*qqqq*/  
  if ( (ent = ini_register_entry("key_qqqq", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_qqqq, "" );

  /*jos*/  
  if ( (ent = ini_register_entry("key_jos", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_jos, "" );

  /*dri*/  
  if ( (ent = ini_register_entry("key_dri", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_dri, "" );

  /*crh*/  
  if ( (ent = ini_register_entry("key_crh", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_crh, "" );

  /*lag*/  
  if ( (ent = ini_register_entry("key_lag", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_lag, "" );

  /*lags*/  
  if ( (ent = ini_register_entry("key_lags", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_lags, "" );

  /*cccc*/  
  if ( (ent = ini_register_entry("key_cccc", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_cccc, "" );
  ///////////////////////////////////////////////////////////////////
  /*ffff*/  
  if ( (ent = ini_register_entry("key_ffff", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_ffff, "" );

  /*ssss*/  
  if ( (ent = ini_register_entry("key_ssss", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_ssss, "" );

  /*grt*/  
  if ( (ent = ini_register_entry("key_grt", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_grt, "" );

  /*gggg*/  
  if ( (ent = ini_register_entry("key_gggg", TYPE_KEY)) != NULL )  
     ini_register_data( ent, &set.key_gggg, "" );

  if ( (ent = ini_register_entry("flooder", TYPE_STRING)) != NULL ) 
    { 
     for ( i = 0; i < 4; i++ ) 
      ini_register_data( ent, set.flooder + i, "" ); 
  }
  ...
  if ( (ent = ini_register_entry("key_crasher", TYPE_KEY)) != NULL )  
	...
	if ((ent = ini_register_entry("CrmpPassword", TYPE_STRING)) != NULL)
	{
		//for (i = 0; i < 10; i++)
			ini_register_data(ent, set.CrmpPassword, "");
	}
	if ((ent = ini_register_entry("proxyIP", TYPE_STRING)) != NULL)
	{
		//for (i = 0; i < 10; i++)
		ini_register_data(ent, set.proxyIP1, "");
	}
	if ((ent = ini_register_entry("proxyPort", TYPE_STRING)) != NULL)
	{
		//for (i = 0; i < 10; i++)
		ini_register_data(ent, set.proxyPort1, "");
	}
