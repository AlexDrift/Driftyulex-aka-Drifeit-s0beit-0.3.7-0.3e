...
const char *g_ErrUnknownAttrib  = "Неизвестный параметр";
const char *g_ErrInvalidAttrib  = "Неправильный парметр";
const char *g_ErrNotGroup       = "Значение не является группой";
const char *g_ErrNoValue        = "Требуемое значение";
const char *g_ErrBadValue       = "Плохое значение";
const char *g_ErrUnknownType    = "Неизвестный тип";
const char *g_ErrNotEnum        = "Должно иметь Тип Enum";
...
{
    assert(_Str!=NULL);
    static const char *ErrStr = "недостижимый";
    char Tmp[1024];
        ...
        if( _Button==TW_MOUSE_LEFT && m_HighlightedLine>=0 && m_HighlightedLine<(int)m_HierTags.size() && m_HierTags[m_HighlightedLine].m_Var )
        {
            ...
            else if( _Pressed && !m_MouseDrag )
            {
                ...
                else if( IsEnumType(Var->m_Type) && !Var->m_ReadOnly && !g_TwMgr->m_IsRepeatingMousePressed )
                {
                    ...
                    g_TwMgr->m_PopupBar = TwNewBar("~ Enum Popup ~", 0);
        ...
        else if( _Pressed && g_TwMgr->m_FontResizable && !m_IsPopupList && _X>=m_PosX+m_Font->m_CharHeight && _X<m_PosX+2*m_Font->m_CharHeight && _Y>m_PosY && _Y<m_PosY+m_Font->m_CharHeight )
        {
            ...
            if( _Button==TW_MOUSE_LEFT )
            {
                if( m_Font==g_DefaultSmallFont )
					g_TwMgr->SetFont(g_DefaultFixedRuFont, true);
				else if (m_Font == g_DefaultFixedRuFont)
